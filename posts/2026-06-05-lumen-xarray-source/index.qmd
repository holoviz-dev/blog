...
and `msl`) and the same chain takes it. Two questions in one query
come back as one tidy table, monthly mean `t2m` in Celsius
alongside monthly mean `msl` in hPa, joined on the time axis:
