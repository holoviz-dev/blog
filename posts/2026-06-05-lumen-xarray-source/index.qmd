...
description: |
  ...
  pipelines. Dimensions, coordinates and attributes are preserved
  through the SQL query, and Dask-backed laziness carries through
  to multi-million-row datasets.
author:
  - name: Aman Kumar
...
[xarray-sql](https://github.com/xarray-contrib/xarray-sql) and
...
anonymous fsspec credentials xarray already accepts. Dimensions,
coordinate units, and attributes survive the round trip, and
Dask-backed laziness carries through to multi-million-row datasets
without materialising the grid.
...
This is the part that matters more than the speed numbers. Every
Lumen pipeline downstream of an XArraySQLSource sees the source's
real schema, not a flattened approximation.

![What survives the SQL query.](images/diagram-4-roundtrip.png){fig-align="center"}
...
- **Coordinate units** (`degrees_north`, `K`, `datetime64[ns]`)
  flow through to the schema description, so the Lumen AI agent and
  any human looking at the table know what the values mean.
- **`attrs`** like `long_name` and `standard_name` ride along on
  the schema metadata and are surfaced in the docstring view of the
  table.
- **Dask laziness** carries from the underlying Zarr or NetCDF
  through xarray-sql into DataFusion's query plan. Filters push
  down; aggregations stream.
...
`Dataset.sel(time=..., lat=...)` is. The XArraySQLSource gives Lumen
a faithful SQL view of the data without pretending the underlying
...
back to `xarray.Dataset` lands ([xarray-sql#58](https://github.com/xarray-contrib/xarray-sql/issues/58)).
